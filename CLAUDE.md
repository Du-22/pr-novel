...
本專案早期(在 web-design-engineer skill 引入之前)的視覺規則
是在缺乏設計指導下做的決定,使用者自評可能帶有 AI 味或不必要的妥協。

授權 web-design-engineer skill 在執行任務時:

1. **主動指出**「配色規範」「UI/UX 規則」「Tailwind 配色」段落裡可能不理想的條目
2. **提出替代方案**(列出 2-3 種選項,說明改動的視覺後果與設計理由)
3. **絕不直接修改 CLAUDE.md / tailwind.config.js / 全域 CSS** — 必須等使用者明確同意
4. 同意後,先在 1-2 個元件 demo 改動效果,確認無誤再擴散到其他元件
5. 任何視覺翻新工作在獨立 branch 進行,Vercel preview 驗證後才合併到 main

### 不開放挑戰的部分(視為刻意決策)

- **紫色作為 brand 主色**(可換色階、可換明度,但不可換色相)
- **「無 emoji 除 Logo 📚」政策**
- **日期格式 YYYY/MM/DD**(不使用相對時間)
- **雙資料來源架構**(novelsHelper 的 mockData + localStorage 合併)
- **檔案開頭註解規範**(檔案名稱 / 路徑 / 用途 三行式)

### 提案的觸發時機

- 使用者主動要求視覺 review 時
- 接到「加新頁面 / 新元件」任務時,在 Step 2(Gather Design Context)
  順便檢視一次相關規則,有疑慮再提出
- **不要在每次小修改都重提同樣的建議** — 提一次被否決後就尊重決定,
  等使用者下次主動要求或專案出現新需求時才重提
