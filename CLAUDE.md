...
| Key | 用途 |
|-----|------|
| `uploadedNovels` | 使用者上傳的小說(只存 metadata,內文在 Firebase Storage) |
...
- **PR 合併後刪遠端 branch**:`git push origin --delete <branch>` + `git remote prune origin`(commit 已在 main 歷史裡,PR 頁面也永遠保留所有資訊)
